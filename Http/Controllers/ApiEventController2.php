...
$date=date("Y-m-d");
	...
	public function prev_Reservold(request $request)
    {
        ...
        $data = Event::where('event_date','<',Carbon::now()->toDateString())->where ('user_id',$request->user_id )->get();
		if( sizeof($data) != 0 ) {
...
public function pending_ReservD(request $request)
    {
        ...
        $data = Event::where('event_date','>',Carbon::now()->toDateString())->where ('status',0)->where('dentist_id',$request->dentist_id)->orwhere('event_date','=',Carbon::now()->toDateString())->get();
		//$this->data['events'] = $data;
		if(!empty($data)){
		...
		if($d->follower_id!=""){
           ...
           ->where('event_date','>',Carbon::now()->toDateString())->where ('status',0)->where('dentist_id',$request->dentist_id)->get();
